...
function getReadyToUseAttributes($atts){
	...
	//if key 'tooltip' is set to 'true' or 'false' it is possible to override default
	if (isset($atts['tooltip']) && ($atts['tooltip'] == "false" || $atts['tooltip'] == "true")) {
		$usrTextAsTooltip = $atts['tooltip'];
		unset($atts['tooltip']);
	}
	...
	$attributeArray = array("usrMaxStars" => $usrMaxStars, "usrStarImage" => $usrStarImage, "usrStarText" => $usrStarText, "usrTextAsTooltip" => $usrTextAsTooltip, "usrCalcAverage" => $usrCalcAverage, "usrStarSize" => $usrStarSize, "usrPreviewImg" => $usrPreviewImg);
	return array("usrAtts" => $atts, "usrAttributeArray" => $attributeArray);
}

//Function to get the image string
function getImageString($ratingValue, $usrStarImage, $usrMaxStars, $usrStarText, $usrTextAsTooltip, $usrPreviewImg, $usrStarSize, $usrAggregatedOutput, $usrAggregationCount, $usrCustomImagesFolder){
  ...
  if($usrStarText == "true" && $usrTextAsTooltip == "true") {
  	$imageString .= '<a class="tooltip">';
  }
  ...
  if ($usrStarText == "true" && $usrTextAsTooltip == "false"){
    //With schema.org SEO
    if(get_option('usrSchemaOrg') == "true"){
      if($usrAggregatedOutput == false)
  ...
  if($usrStarText == "true" && $usrTextAsTooltip == "true") {
  	$imageString .= '<span class="tooltip"><img class="callout" src="'.content_url().'/plugins/universal-star-rating/images/callout.gif" />';
  	//With schema.org SEO
  	if(get_option('usrSchemaOrg') == "true"){
  		if($usrAggregatedOutput == false)
  			$imageString .= '<span itemprop="ratingValue">'.$formattedRatingValue.'</span> / <span itemprop="bestRating">'.$usrMaxStars.'</span>';
...
function updateUSRSettings($usrLang, $usrStarSize, $usrMaxStars, $usrStarText, $usrTextAsTooltip, $usrCalcAverage, $usrPermitShortcodedComments, $usrSchemaOrg, $usrSchemaOrgType, $usrCustomImagesFolder, $usrStarImage){
	...
	//Update text as tooltip option
	update_option("usrTextAsTooltip", $usrTextAsTooltip);
...
function updateUSR(){
	...
	if(get_option("usrVersion") != USR_VERSION){
		#USR 1.10.0 Change of Shortcode behaviour
		if(version_compare(substr(get_bloginfo( 'version' ), 0, 3), '4.4') >= 0){
			$sql = "UPDATE " . $wpdb->prefix . "posts SET post_content = REPLACE ( post_content, 'usr=', 'usr ')";
...
function initUSR(){
	...
	if( !get_option( "usrTextAsTooltip" ) )
		add_option('usrTextAsTooltip', USR_DEFAULT_TEXT_AS_TOOLTIP, '', 'yes');
	...
	if( !get_option( "usrSchemaOrgType" ) )
		add_option('usrSchemaOrgType', USR_DEFAULT_SCHEMA_ORG_TYPE, '', 'yes');

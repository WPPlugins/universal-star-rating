...
/*  Copyright 2013 - 2016  Mike Wigge  (email : wordpress@cizero.de)
...
define('USR_DEFAULT_SCHEMA_ORG_TYPE','reviewRating');
...
if (get_option('usrPermitShortcodedComments') == "true"){
  ...
  global $MESSAGES;
	...
	if (isset($_POST['usrOptionsReset'])) {
		updateUSRSettings(USR_DEFAULT_LANG, USR_DEFAULT_STAR_SIZE, USR_DEFAULT_MAX_STARS,
			USR_DEFAULT_STAR_TEXT, USR_DEFAULT_TEXT_AS_TOOLTIP, USR_DEFAULT_CALC_AVERAGE, USR_DEFAULT_PERMIT_SHORTCODE_COMMENTS,
			USR_DEFAULT_SCHEMA_ORG, USR_DEFAULT_SCHEMA_ORG_TYPE, USR_DEFAULT_CUSTOM_IMAGE_FOLDER, USR_DEFAULT_STAR_IMAGE);
	...
	if (isset($_POST['usrOptionsUpdate'])) {
		...
		$usrTextAsTooltip = get_option("usrTextAsTooltip");
		if(isset($_POST["usrTextAsTooltip"])){
			$usrTextAsTooltip = $_POST["usrTextAsTooltip"];
		}
		...
		updateUSRSettings($usrLang, $usrStarSize, $usrMaxStars, $usrStarText, $usrTextAsTooltip, $usrCalcAverage, $usrPermitShortcodedComments, $usrSchemaOrg, $usrSchemaOrgType, $usrCustomImagesFolder, $usrStarImage);
	...
	?>
		...
		<div class="wrap" style="width: 800px;">		
            ...
            <table border="0">
              ...
              <?php if(get_option( "usrStarText" ) == "true" ){ ?>
              <tr>
              	<td><?php _e($SETTINGS['OPT']['ExplainTextAsTooltip'][$usrLang].":", 'universal-star-rating'); ?></td>
              	<td><?php
                $usrTextAsTooltip = get_option('usrTextAsTooltip');
                
                echo '<select name="usrTextAsTooltip"><option value="true"';
                  if($usrTextAsTooltip == "true"){echo ' selected';}
                echo '>'.$SETTINGS['OPT']['DefaultEnabled'][$usrLang].'</option><option value="false"';
                  if($usrTextAsTooltip == "false"){echo ' selected';}
                echo '>'.$SETTINGS['OPT']['DefaultDisabled'][$usrLang].'</option></select>';
                
      					?></td>
              	<td><?php _e($SETTINGS['OPT']['DefaultTextAsTooltip'][$usrLang], 'universal-star-rating'); ?></td>
              </tr>
              <?php	} ?>
              <tr>
              ...
              <?php if(get_option( "usrSchemaOrg" ) == "true" ){ ?>
              <tr>
              	<td><?php _e($SETTINGS['OPT']['ExplainSchemaOrgType'][$usrLang].":", 'universal-star-rating'); ?></td>
              	<td><?php
                $usrSchemaOrgType = get_option('usrSchemaOrgType');
                
                echo '<select name="usrSchemaOrgType"><option value="rating"';
                  if($usrSchemaOrgType == "rating"){echo ' selected';}
                echo '>rating</option><option value="reviewRating"';
                  if($usrSchemaOrgType == "reviewRating"){echo ' selected';}
                echo '>reviewRating</option></select>';
                
      			?>
      			</td>
              	<td><?php _e($SETTINGS['OPT']['DefaultSchemaOrgType'][$usrLang], 'universal-star-rating'); ?></td>
              </tr>
              <?php	} ?>
              <tr>
          ...
          <p>
            <table border="1" cellspacing="0" cellpadding="5" width="100%">
              ...
              <tr>
                <td><?php _e($SETTINGS['PREV']['ExampleUsrOverriddenTooltipTrue'][$usrLang], 'universal-star-rating'); ?></td>
                <td><?php _e($SETTINGS['PREV']['ExampleUsrResult'][$usrLang].insertUSR(array("3.5", "text" => "true", "tooltip" => "true", "usrPreviewImg" => "true")), 'universal-star-rating'); ?></td>
              </tr>
              <tr>
                <td><?php _e($SETTINGS['PREV']['ExampleUsrOverriddenTooltipFalse'][$usrLang], 'universal-star-rating'); ?></td>
                <td><?php _e($SETTINGS['PREV']['ExampleUsrResult'][$usrLang].insertUSR(array("3.5", "text" => "true", "tooltip" => "false", "usrPreviewImg" => "true")), 'universal-star-rating'); ?></td>
              </tr>
              <tr>
